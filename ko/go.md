...
`select`에 세심히 주의를 기울여 보세요. `c`에는 송신을 하지만 `time.After`로 부터는 수신을 한다는 것에 주목해 주세요. `select`는 송신을 하든 수신을 하든 어떤 채널 조합이든 관계 없이 동일하게 동작합니다:
